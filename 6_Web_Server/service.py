...
from response_helper import make_response, ReturnCode
...
def service_query_capture_log(resquest):
    ...
    results = CaptureLog.query.with_entities(
        CaptureLog.dbid,
    ...
    ).order_by(CaptureLog.capture_datetime).all()
    
    # 將查詢結果轉換為字典
    result_list = [{"dbid": r.dbid,
                    "capture_datetime": datetime.fromisoformat(r.capture_datetime).strftime("%Y-%m-%d %H:%M:%S"),
                    "img_path": img_path_combiner(r.img_path),
                    "predict_probability": r.predict_probability,
                    "class_label": r.class_label} for r in results]
